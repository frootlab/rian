...
class system:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'config' in kwargs \
            ...
            mp.dictMerge(kwargs['config'], self._config)
    ...
    def configure(self, dataset = None, network = None, *args, **kwargs):
        ...
        if not hasattr(self.__class__, '_configure') \
            ...
            return True
        mp.log('info', "configure system '%s'" % (self.getName()))
        mp.setLog(indent = '+1')
        if not self.checkNetwork(network):
            mp.log('error', 'system could not be configured: network is not valid!')
            mp.setLog(indent = '-1')
            return False
        if not self.checkDataset(dataset):
            mp.log('error', 'system could not be configured: dataset is not valid!')
            mp.setLog(indent = '-1')
            return False
    ...
    def _isNetworkMultilayerFeedworward(self, network):
        """Check if the network is a multilayer feedforward network."""
        layers = network.layers()
        if len(layers) < 3:
            mp.log('error', 'Multilayer networks need at least three layers!')
            return False
        firstLayer = layers[0]
        lastLayer = layers[-1]
        for layer in network.layers():
            ...
            if not len(layerAttrib['nodes']):
                mp.log('error', 'Feedforward networks do not allow empty layers!')
                return False
            if layer in [firstLayer, lastLayer]:
                if not layerAttrib['visible']:
                    mp.log('error', 'The first and the last layer of a multilayer feedforward network have to be visible!')
                    return False
            else:
                if layerAttrib['visible']:
                    mp.log('error', 'The middle layers of a multilayer feedforward networks have to be hidden!')
                    return False
        return True
    ...
    def _isDatasetBinary(self, dataset):
        ...
        if not (data == data.astype(bool)).sum() == data.size:
            mp.log('error', 'The dataset does not contain binary data!')
            return False
        return True
    ...
    def _isDatasetGaussNormalized(self, dataset):
        ...
        if numpy.abs(data.mean()) > 0.05 \
            ...
            mp.log('error', 'The dataset does not contain (gauss) normalized data!')
            return False
        return True
