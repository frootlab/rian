...
def plotUnitRelationGraph(model, file = None, **params):
    ...
    if settings['filter'] == None:
        ...
        mp.log("info", "   calculate filter mask: %s > %.1f sigma" % \
        ...
        numFilter = np.sum(M)
        if numFilter == 0:
            mp.log("warning", "   no relation passed filter")
            return False

        mp.log("info", "   %i relations passed filter (%.1f%%)" % \
    ...
    mp.log("info", "   calculate edge weights: " + settings['relation'])
    ...
    # normalize weights
    if np.max(W) == 0:
        mp.log("warning", '   no weights > 0 found')
        return False
    else:
        W = W / np.max(W)
    ...
    numSub = len(Gsub)
    if numSub > 1:
        mp.log("info", "   %i disconnected complexes found" % (numSub))
