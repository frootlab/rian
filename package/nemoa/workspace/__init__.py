...
class __config:
    ...
    def __updateBasepath(self):
        ...
        if 'files' in cfg.sections():
            for key in ['logfile']:
                ...
                self.__path[key] = self.__expandPath(val)
    ...
    def __importShared(self):
        ...
        for workspace in self.__listSharedWorkspaces():
    ...
    def load(self, workspace):
        ...
        if not workspace in self.__listUserWorkspaces(): return nemoa.log(
        ...
        self.__updatePaths(base = 'user') # update paths
        self.__updateCachePaths() # update path of cache
        nemoa.initLogger(logfile = self.__path['logfile']) # update logger
        self.__scanConfigFiles() # import configuration files
        self.__scanScripts() # scan for scriptfiles
        self.__scanNetworks() # scan for network configurations
    ...
    def __scanConfigFiles(self, files = None):
        ...
        for file in glob.iglob(self.__expandPath(files)):
    ...
    def __scanScripts(self, files = None):
    ...
    def __scanNetworks(self, files = None):
    ...
    def __checkNetwork(self, objConf):
        ...
        if 'source' in conf:
            ...
            if not self.__expandPath(file, check = True): return nemoa.log('warning',
                "skipping network '%s': file '%s' not found!" % (name, file))

            objConf['config']['source']['file'] = self.__expandPath(file)
    ...
    def __checkDataset(self, objConf, frac = 1.0, update = True):
        ...
        if 'file' in conf['source']:
            ...
            conf['source']['file'] = self.__expandPath(file)
            conf['type'] = 'file'
        ...
        if 'datasets' in conf['source']:
            ...
            for srcName in srcList:
                ...
                srcObjConf = self.__checkDataset(
    ...
    def __checkSystem(self, objConf):
    ...
    def __checkSchedule(self, objConf):
    ...
    def list(self, type = None, namespace = None):
        ...
        if type == 'workspace': # 2DO: Something wents wrong if list is executed from inside
            return sorted(self.__listUserWorkspaces())
    ...
    def __expandPath(self, str, check = False, create = False):
        ...
        path = self.__expandPathEnv(path) # expand nemoa env vars
        ...
        def __getLayerNetwork(self, file, netcfg, network):
            ...
            for layer in config['layer']:
                ...
                if 'nodes' in netcfg.options(layerSec): nodeList = \
                    ...
                    listFile = nemoa.workspace.__expandPath(
