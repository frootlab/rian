...
def modelaccuracy(model, *args, **kwargs):
    """Mean data reconstruction accuracy of output units."""
    return numpy.mean(unitaccuracy(model, *args, **kwargs))
...
def modelprecision(model, *args, **kwargs):
    """Mean data reconstruction precision of output units."""
    return numpy.mean(unitprecision(model, *args, **kwargs))
...
def unitmean(model, data, mapping = None, block = None):
...
def unitvariance(model, data, mapping = None, block = None):
    ...
    if mapping == None: mapping = model.system._get_mapping()
    if block == None:
        model_out = unitexpect(data, mapping)
    else:
        data_in_copy = numpy.copy(data)
        for i in block:
            data_in_copy[:,i] = numpy.mean(data_in_copy[:,i])
...
def unitexpect(model, *args, **kwargs):
...
def unitvalues(model, *args, **kwargs):
...
def unitresiduals(model, data, mapping = None, block = None):
    ...
    m_out = unitexpect(d_src, mapping)
...
def uniterror(model, data, norm = 'MSE', **kwargs):
...
def unitaccuracy(model, data, norm = 'MSE', **kwargs):
...
def unitprecision(model, data, norm = 'SD', **kwargs):
